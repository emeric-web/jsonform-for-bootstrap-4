  ...
  return '<div ' +
    ...
    '<div class="controls">' +
      ...
      '<div class="<% if (node.prepend) { %>input-group<% } %>' +
        ...
        '<% if (node.prepend) { %>' +
          '<span class="input-group-prepend input-group-text"><%= node.prepend %></span>' +
        '<% } %>' +
      '<% } %>' +
      inner +
      '<% if (node.append) { %>' +
        '<span class="input-group-append input-group-text"><%= node.append %></span>' +
      '<% } %>' +
      '<% if (node.prepend || node.append) { %>' +
        '</div>' +
      '<% } %>' +
      '<% if (node.description) { %>' +
        '<span class="form-text"><%= node.description %></span>' +
      '<% } %>' +
...
var fileDisplayTemplate = '<div class="_jsonform-preview">' +
  ...
  '<% } %>' +
  '</div>' +
  '<a href="#" class="btn btn-secondary _jsonform-delete"><i class="fa fa-times" title="Remove"></i></a> ';
...
jsonform.elementTypes = {
  ...
  'textarea':{
    'template':'<textarea id="<%= id %>" name="<%= node.name %>" ' +
      '<%= (fieldHtmlClass ? "class=\'" + fieldHtmlClass + "\' " : "") %>' +
      'style="height:<%= elt.height || "min-content" %>;width:<%= elt.width || "100%" %>;"' +
      ' aria-label="<%= node.title ? escape(node.title) : node.name %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.readOnly ? " readonly=\'readonly\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.minLength ? " minlength=\'" + node.schemaElement.minLength + "\'" : "") %>' +
  ...
  'checkbox':{
    'template': '<div class="form-check"><input type="checkbox" id="<%= id %>" ' +
      'class=\"form-check-input<%= (fieldHtmlClass ? " fieldHtmlClass": "") %>\"' +
      'name="<%= node.name %>" value="1" <% if (value) {%>checked<% } %>' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
      ' /><label for="<%= id %>"><%= node.inlinetitle || "" %>' +
      '</label></div>',
    'fieldtemplate': true,
    'inputfield': true,
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<a id="<%= node.id %>_dropdown" class="btn dropdown-toggle<% if (buttonClass && node.value) { %> <%= buttonClass %><% } else { %> btn-secondary<% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
        '<% if (node.value) { %><img src="<% if (!node.value.match(/^https?:/)) { %><%= prefix %><% } %><%= node.value %><%= suffix %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu" aria-labelledby="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(node.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } else { %> btn-secondary<% } %>" style="max-width:<%= width %>px;max-height:<%= height %>px"><% if (key instanceof Object) { %><img src="<% if (!key.value.match(/^https?:/)) { %><%= prefix %><% } %><%= key.value %><%= suffix %>" alt="<%= key.title %>" /></a><% } else { %><img src="<% if (!key.match(/^https?:/)) { %><%= prefix %><% } %><%= key %><%= suffix %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
  ...
  'iconselect': {
    'template': '<div>' +
      '<input type="hidden" name="<%= node.name %>" id="<%= node.id %>" value="<%= value %>" />' +
      '<div class="dropdown show">' +
      '<a id="<%= node.id %>_dropdown" class="btn dropdown-toggle<% if (buttonClass && node.value) { %> <%= buttonClass %><% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
        '<% if (node.value) { %><i class="icon-<%= node.value %>" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu navbar" aria-labelledby="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(node.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" ><% if (key instanceof Object) { %><i class="fa fa-<%= key.value %>" alt="<%= key.title %>" /></a><% } else { %><i class="fa fa-<%= key %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
  ...
  'radios':{
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %>' +
      ...
      '<input id="<%= node.id + "-input-" + val %>" class="form-check-input<%= (fieldHtmlClass ? " " + fieldHtmlClass: "") %>" type="radio"' +
      '<% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %>' +
      'name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
      '/><label class="form-check-label" for="<%= node.id + "-input-" + val %>"><%= (key instanceof Object ? key.title : key) %></label></div> <% }); %></div>',
    'fieldtemplate': true,
    'inputfield': true
  },
  'radiobuttons': {
    'template': '<div id="<%= node.id %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<label class="btn btn-secondary <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %>active btn-success<% } %>">' +
  ...
  'checkboxes':{
    ...
    'onBeforeRender': function (data, node) {
      ...
      var template = '<div class="form-check">' +
        '<input type="checkbox" class="form-check-input" <% if (value) { %> checked="checked" <% } %> id="<%= node.id + "-input-" + name %>" name="<%= name %>" value="1"' +
        '<%= (node.disabled? " disabled" : "")%>' +
        '/><label class="form-check-label" for="<%= node.id + "-input-" + name %>"><%= title %></label></div>';
      if (!node || !node.schemaElement) return;
  ...
  'array': {
    'template': '<div id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<span class="_jsonform-array-buttons">' +
        '<a href="#" class="btn btn-secondary _jsonform-array-addmore"><i class="fa fa-plus" title="Add new"></i></a> ' +
        //'<a href="#" class="btn btn-secondary _jsonform-array-deletelast"><i class="fa fa-minus" title="Delete last"></i></a>' +
      '</span>' +
      '</div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner, enableDrag) {
      if ($('').sortable) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          ...
          (enableDrag ? '<span class="draggable line"><i class="fa fa-list" title="Move item"></i></span>' : '') +
          inner +
          '<span class="_jsonform-array-buttons">' +
            '<a href="#" class="btn btn-secondary _jsonform-array-deletecurrent"><i class="fa fa-minus" title="Delete current"></i></a>' +
          '</span>' +
          '</li>';
      }
      else {
    ...
    'onInsert': function (evt, node) {
      ...
      function deleteArrayItem (idx) {
        /* START Emeric */
        /* interception if there is a modal and no data in it */
        const modalConfirmation = $('#modal-confirm-line-suppression')
        if (modalConfirmation && document.getElementById('allowToDelete').value !== 'ok') {
          for (const child of node.children[idx].children) {
            const target = document.getElementById(child.id)
            if (target.value) {
              $('#allowToDelete').val('ok');
              $('#modal-confirm-line-suppression #modal-confirm-line-suppression-ok').off('click')
              modalConfirmation.off('hidden.bs.modal');
              $('#modal-confirm-line-suppression #modal-confirm-line-suppression-ok').one('click', () => {
                $('#modal-confirm-line-suppression').modal('hide');
                deleteArrayItem(idx);
              })
              modalConfirmation.one('hidden.bs.modal', function () {
                $('#allowToDelete').val('no');
              })
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      '</div>' +
      '<a href="#" class="btn btn-secondary _jsonform-array-addmore"><i class="fa fa-plus" title="Add new"></i></a> ' +
      '<a href="#" class="btn btn-secondary _jsonform-array-deleteitem"><i class="fa fa-minus" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
  ...
  'help': {
    'template':'<span class="form-text" style="padding-top:5px"><%= elt.helpvalue %></span>',
    'fieldtemplate': true
  },
  'msg': {
    'template': '<%= elt.msg %>'
  ...
  'button':{
    'template':' <button type="button" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-secondary <%= elt.htmlClass?elt.htmlClass:"" %>"><%= node.title %></button> '
  },
  'actions':{
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="<%= (node.formElement.optionsType === "radiobuttons") ? "btn btn-secondary" : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label> <% }); %></div>',
    'fieldtemplate': true,
    'onInsert': function (evt, node) {
      var activeClass = 'active';
      var elt = node.formElement || {};
